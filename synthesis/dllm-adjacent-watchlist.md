...
## CROSS_PAPER_SYNTHESIS disposition

- `[agent_inference]` Existing intersections are restricted to three mechanism correspondences: adaptive token/block commitment granularity, arbitrary-location code infilling, and diffusion-native outcome training. CodeFusion's globally editable latent is retained as supporting code-construction evidence, not as feedback-driven recovery. Sources: Block Diffusion and Fast-dLLM method sections; Dream Section 5.5.3; d1 Sections 3-4; DiffuCoder Section 5; CodeFusion Sections 3 and 5.3.
- `[agent_inference]` Non-transfer is explicit: a denoising step is not a recovery action without an observed failure that changes revision; marginal token confidence is not calibrated agent-state correctness; masked-token revision does not model externally state-changing tool actions; and code-generation benchmarks do not establish repository navigation, testing, proof control, or repair. Sources: six full readings; `synthesis/cross-paper-synthesis.md / dLLM observation axis`.
- `[agent_inference]` Questions retained for verification are whether generation order has causal task value, whether external test/tool feedback can revise committed regions, whether infilling preserves untouched code and specifications, whether confidence predicts semantic outcomes and recoverability, and whether matched AR comparisons survive common training data and compute. Sources: paper-level evidence gaps and cross-paper verdict above.
- `[agent_inference]` The axis remains adjacent and unpromoted at `CROSS_PAPER_SYNTHESIZED`; no dLLM work is treated as a core SE-agent mechanism, final direction, or presumed replacement for autoregressive agents. Source: current evidence boundary and `state/checkpoint.yaml / cross_paper_synthesis.dllm_axis`.
